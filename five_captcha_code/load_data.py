...
def load_data(tol_num,train_num):
    ...
    df = pd.read_csv("../dataset-5-joined.csv", header=-1)
    i = 0
    for v in df.values:
        img = get_image_from_file('../captcha-5-joined/{}.png'.format(v[0]))
        arr = np.asarray(img, dtype="float32")
        try:
            data[i, :, :, :] = arr
            label[i] = text2vec(v[1])
        except:
            pass
        i = i + 1
...
def load_data_test(tol_num, train_num):
    # input,tol_num: the numbers of all samples(train and test)
    # input,train_num: the numbers of training samples
    # output,(X_train,y_train):trainging data
    # ouput,(X_test,y_test):test data
    ...
    df = pd.read_csv("../dataset-5-test.csv", header=-1)
    i = 0
    for v in df.values:
        img = get_image_from_file(
            '../captcha-5-test/{}.png'.format(v[0]))
        arr = np.asarray(img, dtype="float32")
        try:
            data[i, :, :, :] = arr
            label[i] = text2vec(v[1])
